...
| `keyword_cluster.py` | Raggruppa le keyword in cluster semantici con ML (TF-IDF + K-Means) |
...
### Step 7 — Keyword Clustering semantico
Prendi il CSV prodotto dallo Step 5 (o qualsiasi lista di keyword con colonna `Keyword`),
imposta `FILE_INPUT` e `NUM_CLUSTER` in `keyword_cluster.py`, poi lancia:
```bash
python keyword_cluster.py
```
Legge: `[nome_file].csv` (deve contenere una colonna `Keyword`)
Produce: `[nome_file]_CLUSTERED.csv`

Ogni keyword viene assegnata a un **cluster semantico** (es. *"TEMA: prezzo - confronto"*),
permettendoti di costruire silos SEO o gruppi di annunci per le campagne Ads in pochi secondi.

> 💡 **Suggerimento:** imposta `NUM_CLUSTER` tra 5 e 10 per dataset di poche centinaia di keyword.
> Per volumi più grandi (1.000+), prova valori tra 10 e 20 e valuta i cluster generati prima di usarli.
...
├── keyword_cluster.py        # Step 7: clustering semantico ML (TF-IDF + K-Means)
│
├── backlink/
...
MIT License — libero per uso personale e commerciale.# 🔍 SEO Automation Suite
